...
$sql = sprintf("SELECT COUNT(employeenumber) AS 'num' FROM user u WHERE employeenumber='%d' AND NOT EXISTS (SELECT employeenumber FROM member WHERE employeenumber = '%d')",FSQL($_POST["employee"]),"","",FSQL($_POST["employee"]));
$query = mysql_query($sql,$db);
$ob = mysql_fetch_object($query);

if($ob->num == 1) echo "OKACTIVATED"; 
else echo "NOACTIVATED";
}
mysql_close($db);
