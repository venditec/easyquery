<?php
/*
.---------------------------------------------------------------------------.
|  Software: EasyQuery                                  					|
|   Version: 1.0                                                            |
|   Contact: via sourceforge.net or ramon.sepi@gmail.com                    |
|      Info: http://sourceforge.net/projects/easyquery1/                    |
|   Support: http://sourceforge.net/projects/easyquery1/                    |
| ------------------------------------------------------------------------- |
|     Admin: Ramon Segarra Pijuan (project admininistrator)                 |
|   Author : Ramon Segarra Pijuan 											|
|   Founder: Ramon Segarra Pijuan (original founder)                        |
| Copyright (c) 2012-2013, Ramon Segarra Pijuan. All Rights Reserved.       |
| ------------------------------------------------------------------------- |
|   License: Distributed under the Creative Commons 						|
|            Attribution-NonCommercial 3.0 Unported License.     			|
|            http://creativecommons.org/licenses/by-nc/3.0/                 |
| This program is distributed in the hope that it will be useful -          |
| YOU ARE FREE: to Share — to copy, distribute and transmit the work.       |
|               to Remix — to adapt the work.								|
| UNDER THE FOLLOWING CONDITIONS: 											|
|		Noncomercial - You may not use this work for commercial purposes.	|
|		Attribution  — You must attribute the work to Ramon Segarra Pijuan	|
|                      like the author or licensor (but not in any way that |
|					   suggests that they endorse you or your use 			|
|                      of the work).										|
| ------------------------------------------------------------------------- |
|          EasyQuery by Ramon Segarra Pijuan is licensed under a 			|
|     Creative Commons Attribution-NonCommercial 3.0 Unported License.		|
'---------------------------------------------------------------------------'
*/


include("../_etc/db/dbConect.php");
/*
 * This is the security code to recive the request from client
 * Put the same security code of SecurityCode.java
 */ 
$SECURITYCODE = "12345";

$db = dbConect();
if($_POST["codeW"]==$SECURITYCODE){
if(!empty($_POST["password"])){
	$sql = sprintf("SELECT COUNT(employeenumber) AS 'num' FROM user WHERE employeenumber='%s' AND employeepassword='%s'",FSQL($_POST["employee"]),sha1($_POST["password"]));
	$query = mysql_query($sql,$db);
	$ob = mysql_fetch_object($query);
	$sql1 = sprintf("SELECT * FROM user WHERE employeenumber='%s' AND employeepassword='%s'",FSQL($_POST["employee"]),sha1($_POST["password"]));
	$query1 = mysql_query($sql1,$db);
	$ob1 = mysql_fetch_object($query1);
	
	if($ob->num == 1 && $ob1->activated == 0) {
		$sql2 = sprintf("SELECT * FROM user WHERE employeenumber='%s' AND employeepassword='%s'",FSQL($_POST["employee"]),sha1($_POST["password"]));
		$query2 = mysql_query($sql2,$db);
		$ob2 = mysql_fetch_object($query2);
		echo "OKAUTH|".$ob2->role; 
	}else if($ob->num == 1 && $ob1->activated == 1){
		$sql3 = sprintf("UPDATE user SET activated = %d WHERE employeenumber = %d",0,$ob1->employeenumber);
		$query3 = mysql_query($sql3,$db);
		usleep(10000000);
		$sql4 = sprintf("SELECT * FROM user WHERE employeenumber='%s' AND employeepassword='%s'",FSQL($_POST["employee"]),sha1($_POST["password"]));
		$query4 = mysql_query($sql4,$db);
		$ob4 = mysql_fetch_object($query4);
		echo "OKAUTH|".$ob4->role; 
	}
	else echo "NOAUTH";
}else{
	echo "NOAUTH";
}
}
mysql_close($db);

?>