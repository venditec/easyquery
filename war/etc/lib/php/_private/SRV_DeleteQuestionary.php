...
$sql = sprintf("DELETE FROM assign WHERE questionary_id='%d'",FSQL($_POST["idquestionary"]));
$query = mysql_query($sql,$db);
$sql = sprintf("DELETE q.*, a.* FROM question q, answer a WHERE q.questionary_id='%d' AND q.id=a.question_id",FSQL($_POST["idquestionary"]));
$query = mysql_query($sql,$db);
$sql = sprintf("DELETE r.*, t.* FROM reply r, result t WHERE r.questionary_id='%d' AND r.id=t.reply_id",FSQL($_POST["idquestionary"]));
$query = mysql_query($sql,$db);
$sql = sprintf("DELETE FROM questionary WHERE id='%d'",FSQL($_POST["idquestionary"]));
$query = mysql_query($sql,$db);
echo "OKDELETEQUESTIONARY";
}	
mysql_close($db);
