...
$sql = sprintf("SELECT g.* FROM questionary q, assign a, groupp g WHERE a.questionary_id = q.id AND a.group_id=g.id AND q.id='%d' ORDER BY g.name",FSQL($_POST["idquestionary"]));
$query = mysql_query($sql,$db);
while($ob = mysql_fetch_object($query)){
	$result = $result.$ob->id."|".$ob->name."#";
}
