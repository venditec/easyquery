...
$sql = sprintf("SELECT * FROM question WHERE id = '%d' ORDER BY id",FSQL($_POST["idquestion"]));
$query = mysql_query($sql,$db);
$ob = mysql_fetch_object($query);
$result = $result.$ob->id."|".$ob->text."#";
	
$sql1 = sprintf("SELECT * FROM answer WHERE question_id = '%d' ORDER BY id",FSQL($_POST["idquestion"]));
$query1 = mysql_query($sql1,$db);
while($ob1 = mysql_fetch_object($query1)){
	$result = $result.$ob1->id."|".$ob1->text."#";
}
