...
if(!empty($_POST["employee"]) && !empty($_POST["oldpassw"]) && !empty($_POST["newpassw"])){
	$sql = sprintf("SELECT * FROM user WHERE employeenumber='%s'",FSQL($_POST["employee"]));
	$query = mysql_query($sql,$db);
	$ob = mysql_fetch_object($query);
	if(sha1($_POST["oldpassw"]) == $ob->employeepassword){
		$sql = sprintf("UPDATE user SET employeepassword = '%s' WHERE employeenumber = '%d'",sha1($_POST["newpassw"]),FSQL($_POST["employee"]));
		$query = mysql_query($sql,$db);
		echo "OKCHANGE";
	}else{
		echo "ERROROLDPASSW";
