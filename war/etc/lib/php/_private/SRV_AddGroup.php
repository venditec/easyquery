...
if($_POST["codeW"]==$SECURITYCODE){
	...
	$sql = sprintf("SELECT COUNT(id) AS 'num' FROM groupp WHERE name='%s'",FSQL($_POST["groupname"]));
	$query = mysql_query($sql,$db);
	$ob = mysql_fetch_object($query);
	if($ob->num == 0){
		$sql = sprintf("INSERT INTO groupp (name) VALUES ('%s')",FSQL($_POST["groupname"]));
		$query = mysql_query($sql,$db);
		echo "OKADDGROUP";
	}else{
		echo "ERRORGROUPEXIST";
	}
}else{
	echo "ERRORFIELDS";
}
