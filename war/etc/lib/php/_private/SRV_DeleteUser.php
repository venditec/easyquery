...
$sql = sprintf("SELECT * FROM user WHERE employeenumber='%d'",FSQL($_POST["employee"]));
$query = mysql_query($sql,$db);
$ob = mysql_fetch_object($query);
if($ob->mail == null){
	$sql = sprintf("DELETE FROM user WHERE employeenumber='%d'",FSQL($_POST["employee"]));
	$query = mysql_query($sql,$db);
	echo "OKDELETEUSER";
}else{
	$sql = sprintf("DELETE FROM user WHERE employeenumber='%d'",FSQL($_POST["employee"]));
	$query = mysql_query($sql,$db);
	$sql = sprintf("DELETE FROM member WHERE employeenumber='%d'",FSQL($_POST["employee"]));
	$query = mysql_query($sql,$db);
	echo "OKDELETEUSER";
}
}		
