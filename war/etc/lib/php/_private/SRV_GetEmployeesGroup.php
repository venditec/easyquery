...
$sql = sprintf("SELECT m.* FROM user u, member m WHERE u.employeenumber = m.employeenumber AND u.group_id = '%s' AND u.mail IS NOT NULL ORDER BY m.surname",FSQL($_POST["idgroup"]));
$query = mysql_query($sql,$db);
while($ob = mysql_fetch_object($query)){
	$result = $result.$ob->name."|".$ob->surname."|".$ob->employeenumber."#";
}
$sql = sprintf("SELECT * FROM user WHERE group_id = '%s' AND mail IS NULL ORDER BY employeenumber",FSQL($_POST["idgroup"]));
$query = mysql_query($sql,$db);
while($ob = mysql_fetch_object($query)){
	$result = $result."USER_NOT_ACTIVATED:|".$ob->employeenumber."|".$ob->employeenumber."#";
}
if(strlen($result)>3)
