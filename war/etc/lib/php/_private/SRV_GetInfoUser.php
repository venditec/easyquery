...
$sql = sprintf("SELECT m.*, u.* FROM member m, user u WHERE u.employeenumber='%s' AND u.employeenumber=m.employeenumber",FSQL($_POST["employee"]));
$query = mysql_query($sql,$db);
$ob = mysql_fetch_object($query);

if($ob->name!="" && $ob->surname!=""){
	echo "OK|".$ob->name."|".$ob->surname."|".$ob->mail."|".$ob->privatemail."|".$ob->address."|".$ob->city."|".$ob->postalcode."|".$ob->country."|".$ob->phone."|".$ob->activated;
}
}	
mysql_close($db);
