...
if(!empty($_POST["employeenumber"]) && !empty($_POST["group"])){
	$sql = sprintf("SELECT COUNT(employeenumber) AS 'num' FROM user WHERE employeenumber='%d'",FSQL($_POST["employeenumber"]));
	...
	if($ob->num == 0){
		...
		$sql = sprintf("INSERT INTO user (employeenumber,group_id,role) VALUES ('%d','%d','%s')",FSQL($_POST["employeenumber"]),FSQL($_POST["group"]),$role);
		$query = mysql_query($sql,$db);
		echo "OKREGISTERUSER";
	}else{
		echo "ERRORUSEREXIST";
