...
$sql = sprintf("SELECT count(*) as 'num' FROM user WHERE group_id='%d'",FSQL($_POST["id"]));
$query = mysql_query($sql,$db);
$ob = mysql_fetch_object($query);
if($ob->num == 0){
	$sql = sprintf("DELETE FROM assign WHERE group_id='%d'",FSQL($_POST["id"]));
	$query = mysql_query($sql,$db);
	$sql = sprintf("DELETE FROM groupp WHERE id='%d'",FSQL($_POST["id"]));
	$query = mysql_query($sql,$db);
	echo "OKDELETEGROUP";
}else{
	echo "USERSINGROUP";
