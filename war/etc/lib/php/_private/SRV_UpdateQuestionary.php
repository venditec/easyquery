...
if(!empty($_POST["name"]) && !empty($_POST["startdate"]) && !empty($_POST["enddate"])){
	$sql = sprintf("UPDATE questionary SET name='%s', description='%s', startdate='%s', enddate='%s' WHERE id='%d'",FSQL($_POST["name"]),$_POST["description"],FSQL($_POST["startdate"]),FSQL($_POST["enddate"]),FSQL($_POST["idquestionary"]));
	$query = mysql_query($sql,$db);
	echo "OKUPDATEQUESTIONARY";
}else{
	echo "ERRORFIELDS";
}
}
