...
if($_POST["groupid"] == "0")
$sql = sprintf("SELECT u.*, m.* FROM user u, member m WHERE u.employeenumber = m.employeenumber ORDER BY u.role,u.employeenumber");
else 
$sql = sprintf("SELECT u.*, m.* FROM user u, member m WHERE u.employeenumber = m.employeenumber AND u.group_id=%d ORDER BY u.role,u.employeenumber",FSQL($_POST["groupid"]));

$query = mysql_query($sql,$db);
while($ob = mysql_fetch_object($query)){
	$result = $result.$ob->employeenumber."|".$ob->role."|".$ob->name."|".$ob->surname."|".$ob->mail."|".$ob->activated."#";
}

if($_POST["groupid"] == "0")
$sql = sprintf("SELECT * FROM user WHERE mail IS NULL ORDER BY role,employeenumber");
else
$sql = sprintf("SELECT * FROM user WHERE group_id=%d AND mail IS NULL ORDER BY role,employeenumber",FSQL($_POST["groupid"]));

$query = mysql_query($sql,$db);
while($ob = mysql_fetch_object($query)){
	$result = $result.$ob->employeenumber."|".$ob->role."|-|-|-|0#";
}
if(strlen($result)>3)
