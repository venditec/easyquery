...
$sql = sprintf("UPDATE question SET text='%s' WHERE id='%d'",$_POST["questiontext"],FSQL($_POST["idquestion"]));
$query = mysql_query($sql,$db);

$idquestion = $_POST["idquestion"];

for($i=0;$i<$_POST["numanswers"];$i++){
	$sql = sprintf("UPDATE answer SET text='%s' WHERE id='%d'",$_POST["answertext".$i],$_POST["answerid".$i]);
	$query = mysql_query($sql,$db);
}
echo "OKUPDATEQUESTION";
}
mysql_close($db);
