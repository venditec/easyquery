...
public class ListQuestionary extends VerticalPanel {
	
	public ListQuestionary(){
		setWidth("100%");
						...
						for(int x=1;x<Rows.length;x++){
							...
							Edit.addClickHandler(new ClickHandler() {
								...
								public void onClick(ClickEvent event) {
									MainPanel.getSingleton().Load_EditQuestionary(IdQuestionary.getValue());
								}
							});
							...
							Employees.addClickHandler(new ClickHandler() {
								...
								@Override
								public void onClick(ClickEvent event) {
									MainPanel.getSingleton().Load_QuestionaryAssign(IdQuestionary.getValue(), TEXT1.getText());
								}
							});
							...
							PublicQuestionary.addClickHandler(new ClickHandler() {
								...
								public void onClick(ClickEvent event) {
									ErrorPanel.setVisible(false);
									RequestBuilder REQ = new RequestBuilder(RequestBuilder.POST,URL.encode(GWT.getHostPageBaseURL() + "etc/lib/php/_private/SRV_LockQuestionary.php"));
									REQ.setHeader("Content-Type", "application/x-www-form-urlencoded");
									try {
								...
								public void onClick(ClickEvent event) {
									...
									if(Lock.getValue().contains("1")){
										if(Window.confirm(new Text().getText().Action_Delete())){
											RequestBuilder REQ = new RequestBuilder(RequestBuilder.POST,URL.encode(GWT.getHostPageBaseURL() + "etc/lib/php/_private/SRV_DeleteQuestionary.php"));
											REQ.setHeader("Content-Type", "application/x-www-form-urlencoded");
											try {
												REQ.sendRequest("codeW="+new SecurityCode().getSRVSecurityCode()+"&idquestionary="+IdQuestionary.getValue(), new RequestCallback() {
													...
													public void onResponseReceived(Request request, Response response) {
														//Window.alert(response.getText());
														if(response.getText().equals("OKDELETEQUESTIONARY")){
															MainPanel.getSingleton().Load_QuestionaryList();
														}else{
															ErrorPanel.setTextError(new Text().getText().ErrorServer());
															ErrorPanel.setVisible(true);
