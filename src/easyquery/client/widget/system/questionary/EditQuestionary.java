...
public class EditQuestionary extends VerticalPanel {

	public EditQuestionary(final String IdQuestionary){
					...
					if(TXB2.getText().length()==10 && TXB3.getText().length()==10){
						if(TXB2.getText().charAt(2) == '/' && TXB2.getText().charAt(5) == '/' && TXB3.getText().charAt(2) == '/' && TXB3.getText().charAt(5) == '/'){
							if(new DateQuery(TXB3.getText()).isMoreBigThan(new DateQuery(TXB2.getText())) || new DateQuery(TXB3.getText()).isTheSame(new DateQuery(TXB2.getText()))){
								RequestBuilder REQ = new RequestBuilder(RequestBuilder.POST,URL.encode(GWT.getHostPageBaseURL() + "etc/lib/php/_private/SRV_UpdateQuestionary.php"));
								REQ.setHeader("Content-Type", "application/x-www-form-urlencoded");
								try {
									REQ.sendRequest("codeW="+new SecurityCode().getSRVSecurityCode()+"&name="+TXB1.getText()+"&description="+TXA1.getText()+"&startdate="+TXB2.getText()+"&enddate="+TXB3.getText()+"&idquestionary="+IdQuestionary, new RequestCallback() {
										
										@Override
										public void onResponseReceived(Request request, Response response) {
											SBM.Loading(false);
											if(response.getText().equals("OKUPDATEQUESTIONARY")){
												Window.alert(new Text().getText().SuccesUpdateQuestionary());
		...
		RequestBuilder REQ = new RequestBuilder(RequestBuilder.POST,URL.encode(GWT.getHostPageBaseURL() + "etc/lib/php/_private/SRV_GetQuestionary.php"));
		REQ.setHeader("Content-Type", "application/x-www-form-urlencoded");
		try {
			REQ.sendRequest("codeW="+new SecurityCode().getSRVSecurityCode()+"&idquestionary="+IdQuestionary, new RequestCallback() {
				...
				public void onResponseReceived(Request request, Response response) {
					...
					if(response.getText().split("\\#")[0].equals("OK")){
						String[] value = response.getText().split("\\#")[1].split("\\|");
						
						final Hidden IdQuestionary = new Hidden();
						IdQuestionary.setValue(value[0]);
						TXB1.setText(value[1]);
						TXB2.setText(value[2]);
						TXB3.setText(value[3]);
						TXA1.setText(value[4]);
