...
public class ListQuestionaryForMembers extends VerticalPanel {

	public ListQuestionaryForMembers(){
		setWidth("100%");
		...
		RequestBuilder REQ = new RequestBuilder(RequestBuilder.POST,URL.encode(GWT.getHostPageBaseURL() + "etc/lib/php/_private/SRV_GetQuestionariesForMembers.php"));
		REQ.setHeader("Content-Type", "application/x-www-form-urlencoded");
		try {
			REQ.sendRequest("codeW="+new SecurityCode().getSRVSecurityCode()+"&employee="+InfoUser.getSingleton().EmployeeNumber, new RequestCallback() {
				...
				public void onResponseReceived(Request request, Response response) {
					...
					if(response.getText().split("\\#")[0].equals("OK")){
						...
						String date = response.getText().split("\\#")[1];
						...
						for(int x=2;x<Rows.length;x++){
							...
							Image Pen = new Image(new Icon().Pen);
							Pen.setTitle(new Text().getText().Action_MakeTest());
							Pen.setStyleName("Icon");
							Pen.addClickHandler(new ClickHandler() {
								
								@Override
								public void onClick(ClickEvent event) {
									MainPanel.getSingleton().Load_DoMakeTest(InfoUser.getSingleton().EmployeeNumber, IdQuestionary.getValue(), TEXT1.getText());
								}
							});
							if(!new DateQuery(value[2]).isMoreBigThan(new DateQuery(date)) && !new DateQuery(date).isMoreBigThan(new DateQuery(value[3]))) Pen.setVisible(true);
							else  Pen.setVisible(false);
							Row.Options.add(Pen);
							Row.Options.setCellHorizontalAlignment(Pen, ALIGN_CENTER);
