...
public class AsignUserToGroup extends VerticalPanel {

	private Error ErrorPanel;
	private ListBox LIST1;
	private ListBox LIST2;
	
	public AsignUserToGroup(final String IDGroup, String NameGroup){
		...
		Label Text1 = new Label(new Text().getText().List_EmployeeNumber());
		Text1.setStyleName("TextListHeaderRow");
		Label Text2 = new Label(new Text().getText().EmployeesAtGroup()+" "+NameGroup);
		...
		LIST1.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				ErrorPanel.setVisible(false);
				RequestBuilder REQ = new RequestBuilder(RequestBuilder.POST,URL.encode(GWT.getHostPageBaseURL() + "etc/lib/php/_private/SRV_AsignUserToGroup.php"));
				REQ.setHeader("Content-Type", "application/x-www-form-urlencoded");
				try {
					REQ.sendRequest("codeW="+new SecurityCode().getSRVSecurityCode()+"&idgroup="+IDGroup+"&employee="+LIST1.getValue(LIST1.getSelectedIndex()), new RequestCallback() {
	...
	public void LoadList1(){
		LIST1.clear();
		RequestBuilder REQ1 = new RequestBuilder(RequestBuilder.POST,URL.encode(GWT.getHostPageBaseURL() + "etc/lib/php/_private/SRV_GetEmployeesName.php"));
		REQ1.setHeader("Content-Type", "application/x-www-form-urlencoded");
		try {
			REQ1.sendRequest("codeW="+new SecurityCode().getSRVSecurityCode(), new RequestCallback() {
				...
				public void onResponseReceived(Request request, Response response) {
					...
					if(response.getText().split("\\#")[0].equals("OK")){
						for(int x=1;x<response.getText().split("\\#").length;x++){
							...
							LIST1.addItem(value.split("\\|")[0]+ " " + value.split("\\|")[1],value.split("\\|")[2]);
						}
					}else{
						ErrorPanel.setTextError(new Text().getText().ErrorServer());
						ErrorPanel.setVisible(true);
	...
	public void LoadList2(String IDGroup){
		LIST2.clear();
		RequestBuilder REQ2 = new RequestBuilder(RequestBuilder.POST,URL.encode(GWT.getHostPageBaseURL() + "etc/lib/php/_private/SRV_GetEmployeesGroup.php"));
		REQ2.setHeader("Content-Type", "application/x-www-form-urlencoded");
		try {
			REQ2.sendRequest("codeW="+new SecurityCode().getSRVSecurityCode()+"&idgroup="+IDGroup, new RequestCallback() {
				...
				public void onResponseReceived(Request request, Response response) {
					...
					if(response.getText().split("\\#")[0].equals("OK")){
						for(int x=1;x<response.getText().split("\\#").length;x++){
							String value = response.getText().split("\\#")[x];
							LIST2.addItem(value.split("\\|")[0]+ " " + value.split("\\|")[1],value.split("\\|")[2]);
						}
					}else{
						ErrorPanel.setTextError(new Text().getText().ErrorServer());
						ErrorPanel.setVisible(true);
