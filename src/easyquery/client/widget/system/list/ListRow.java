...
public class ListRow extends HorizontalPanel {
	
	public HorizontalPanel Options;

	public ListRow(Widget[] Columns){
		
		setWidth("100%");
		setStyleName("ListRow");
		Options = new HorizontalPanel();
		Options.setWidth("100%");

		for(int i=0;i<Columns.length;i++){
			Widget Value = Columns[i];
			add(Value);
			setCellHorizontalAlignment(Value, ALIGN_CENTER);
		}
		
		add(Options);
		setCellHorizontalAlignment(Options, ALIGN_CENTER);
		setCellWidth(Options, "20%");
	}
}
